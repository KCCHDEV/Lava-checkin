...
@section('title', 'เช็คชื่อนักเรียน - ระบบเช็คชื่อการมาเรียน')

@section('page-title', 'เช็คชื่อนักเรียน')
...
.student-card input[type="checkbox"]:checked + label {
    color: #667eea;
}

.student-card input[type="checkbox"]:checked ~ .student-card {
    border-color: #667eea;
...
@section('content')
<!-- Single Student Attendance Section -->
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">
                    <i class="fas fa-user-check me-2"></i>เช็คชื่อนักเรียนรายบุคคล
                </h5>
            </div>
            <div class="card-body">
...
<!-- Multiple Students Attendance Section -->
<div class="row mt-4">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('attendances.bulk-store') }}">
                    @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Sync date and time fields between single and bulk sections
    const singleDate = document.getElementById('date');
    const bulkDate = document.getElementById('bulk_date');
    const singleTime = document.getElementById('time');
    const bulkTime = document.getElementById('bulk_time');
    
    // Sync date from single to bulk
    singleDate.addEventListener('change', function() {
        bulkDate.value = this.value;
    });
    
    // Sync date from bulk to single
    bulkDate.addEventListener('change', function() {
        singleDate.value = this.value;
    });
    
    // Sync time from single to bulk
    singleTime.addEventListener('change', function() {
        bulkTime.value = this.value;
    });
    
    // Sync time from bulk to single
    bulkTime.addEventListener('change', function() {
        singleTime.value = this.value;
    });
    ...
    if (!singleDate.value) singleDate.value = currentDate;
    if (!bulkDate.value) bulkDate.value = currentDate;
    if (!singleTime.value) singleTime.value = currentTime;
    if (!bulkTime.value) bulkTime.value = currentTime;
